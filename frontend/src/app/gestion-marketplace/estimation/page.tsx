  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        {/* Header */}
        <div className="mb-8">
          <Link href="/gestion-marketplace" className="inline-flex items-center gap-2 text-muted-foreground dark:text-gray-400 hover:text-amber-400 mb-4 transition-colors">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground dark:text-white">Estimation intelligente</h1>
              <p className="text-foreground dark:text-gray-500">Décrivez votre projet ou uploader une photo pour une estimation automatique</p>
            </div>
          </div>
        </div>
          ...
          <button
            ...
            className={`flex-1 py-4 rounded-xl border transition-all ${
              mode === 'besoin'
                ...
                : 'bg-black/5 dark:bg-white/5 border-border dark:border-white/10 text-muted-foreground dark:text-gray-400 hover:text-foreground dark:text-white'
            }`}
          >
        ...
        {/* Formulaire */}
        <div className="rounded-2xl border border-border dark:border-white/10 bg-black/5 dark:bg-white/5 p-6 mb-8">
          {mode === 'besoin' ? (
            <div className="space-y-6">
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-xl bg-gray-900/80 border border-border dark:border-white/10 px-4 py-3 text-foreground dark:text-white placeholder:text-foreground dark:text-gray-500 focus:outline-none focus:border-amber-500/50"
                  rows={4}
                />
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl bg-gray-900/80 border border-border dark:border-white/10 px-4 py-3 text-foreground dark:text-white"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted-foreground dark:text-gray-300 mb-2">
            ...
            <div className="text-center">
              <div 
                ...
                className="border-2 border-dashed border-border dark:border-white/20 rounded-xl p-8 cursor-pointer hover:border-amber-500/50 transition-all"
              >
                {image ? (
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 p-1 rounded-full bg-red-500 text-foreground dark:text-white"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <>
                    <Upload className="w-12 h-12 mx-auto text-foreground dark:text-gray-500 mb-3" />
                    <p className="text-muted-foreground dark:text-gray-400">Cliquez ou glissez une photo de votre chantier</p>
                    <p className="text-xs text-foreground dark:text-gray-500 mt-2">JPG, PNG jusqu'à 5MB</p>
                  </>
                )}
              </div>
              <input
        ...
        {result && (
          ...
          >
            ...
            <div className="rounded-2xl border border-border dark:border-white/10 bg-black/5 dark:bg-white/5 p-6">
              <h2 className="text-xl font-bold text-foreground dark:text-white mb-4">📊 Résultat de l'estimation</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30">
                  <p className="text-sm text-muted-foreground dark:text-gray-400">Budget estimé</p>
                  <p className="text-2xl font-bold text-emerald-400">{result.estimation?.prix_total?.toFixed(2) || 0} TND</p>
                </div>
                <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/30">
                  <p className="text-sm text-muted-foreground dark:text-gray-400">Main d'œuvre estimée</p>
                  <p className="text-2xl font-bold text-blue-400">{result.estimation?.main_oeuvre?.toFixed(2) || 0} TND</p>
                </div>
                <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30">
                  <p className="text-sm text-muted-foreground dark:text-gray-400">Frais de livraison</p>
                  <p className="text-2xl font-bold text-amber-400">{result.estimation?.frais_livraison?.toFixed(2) || 50} TND</p>
                </div>
              </div>
              {result.surface_estimee && (
                <p className="text-sm text-foreground dark:text-gray-500 mt-4">📐 Surface estimée: {result.surface_estimee} m²</p>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-border dark:border-white/10 bg-black/5 dark:bg-white/5 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-foreground dark:text-white">🛒 Produits recommandés</h2>
                <button
              ...
              <div className="space-y-3">
                {result.produits_recommandes?.map((product: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between p-4 rounded-xl bg-black/5 dark:bg-white/5 border border-border dark:border-white/10">
                    <div className="flex-1">
                      <p className="font-medium text-foreground dark:text-white">{product.nom}</p>
                      <div className="flex items-center gap-4 text-sm text-muted-foreground dark:text-gray-400 mt-1">
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className="flex-1 py-3 rounded-xl bg-black/5 dark:bg-white/5 border border-border dark:border-white/10 text-muted-foreground dark:text-gray-300 hover:text-foreground dark:text-white"
              >
                Nouvelle estimation
              </button>
            </div>
