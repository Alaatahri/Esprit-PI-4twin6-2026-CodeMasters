  ...
  if (!isMounted) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 flex items-center justify-center">
        <div className="text-muted-foreground dark:text-gray-400">Chargement...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="mb-8">
          <button
            onClick={handleBack}
            className="flex items-center gap-2 text-muted-foreground dark:text-gray-400 hover:text-amber-400 transition-colors mb-4"
          >
            <ArrowLeft className="w-4 h-4" />
            Retour
          </button>
          <h1 className="text-2xl font-bold text-foreground dark:text-white">Finaliser la commande</h1>
          <p className="text-foreground dark:text-gray-500 mt-1">Livraison et paiement sécurisé</p>
        </div>
            ...
            <div key={stepItem.num} className="flex items-center">
              <div
                className={`flex items-center justify-center w-10 h-10 rounded-full transition-all ${
                  step >= stepItem.num
                    ...
                    : 'bg-black/5 dark:bg-white/10 text-foreground dark:text-gray-500'
                }`}
              >
                {step > stepItem.num ? (
              ...
              <span
                className={`ml-2 text-sm font-medium hidden sm:inline ${
                  step >= stepItem.num ? 'text-foreground dark:text-white' : 'text-foreground dark:text-gray-500'
                }`}
              >
                {stepItem.label}
              </span>
              {stepItem.num < 3 && (
                <div className="w-12 h-px bg-black/5 dark:bg-white/10 mx-3 hidden sm:block" />
              )}
            </div>
          ))}
        </div>
            ...
            <div className="rounded-2xl border border-border dark:border-white/10 bg-black/5 dark:bg-white/5 p-6">
              ...
              {dropoffLocation && (
                <div className="mt-4 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30">
                  ...
                  <p className="text-foreground dark:text-white text-sm mt-1">
                    Latitude: {dropoffLocation.lat.toFixed(6)}<br />
                    Longitude: {dropoffLocation.lng.toFixed(6)}
                  </p>
                  <p className="text-muted-foreground dark:text-gray-400 text-xs mt-2">
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div className="rounded-2xl border border-border dark:border-white/10 bg-black/5 dark:bg-white/5 p-6">
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-foreground dark:text-gray-500">Aucun véhicule disponible pour le moment</p>
                ...
                <div className="mt-6 p-4 rounded-xl bg-amber-500/10 border border-amber-500/30">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground dark:text-gray-400">Distance</span>
                      <span className="text-foreground dark:text-white">{distance.toFixed(1)} km</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground dark:text-gray-400">Livreur</span>
                      <span className="text-foreground dark:text-white">{selectedDriver.nom}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground dark:text-gray-400">Véhicule</span>
                      <span className="text-foreground dark:text-white">{selectedDriver.vehicule.type}</span>
                    </div>
                    <div className="flex justify-between pt-2 border-t border-amber-500/20">
                      <span className="font-medium text-foreground dark:text-white">Prix livraison</span>
            ...
            <div className="flex justify-between">
              <button
                onClick={handleBack}
                className="flex items-center gap-2 px-6 py-3 rounded-xl bg-black/5 dark:bg-white/5 border border-border dark:border-white/10 text-muted-foreground dark:text-gray-300 hover:text-foreground dark:text-white hover:bg-black/5 dark:bg-white/10 transition-all"
              >
                <ArrowLeft className="w-4 h-4" />
                Retour
              </button>
