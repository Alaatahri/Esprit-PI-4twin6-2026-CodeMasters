...
export default function MarketplacePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950">
      {/* Header */}
      <div className="sticky top-0 z-40 bg-gray-950/90 backdrop-blur-xl border-b border-white/10">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-4">
              <Link 
                href="/" 
                className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-amber-400 hover:border-amber-500/30 transition-all duration-300"
              >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-white">Marketplace BMP.tn</h1>
                  <p className="text-xs text-gray-500">Matériaux de construction professionnels</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                ...
                className="hidden md:flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-amber-400 hover:border-amber-500/30 transition-all duration-300 text-sm font-medium"
              >
              ...
              <Link
                ...
                className="hidden md:flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white hover:shadow-lg hover:shadow-purple-500/30 transition-all duration-300 text-sm font-medium"
              >
              ...
              <button
                ...
                className={`p-2.5 rounded-xl transition-all duration-300 ${
                  showFilters 
                    ...
                    : 'bg-white/5 border border-white/10 text-gray-400 hover:text-amber-400 hover:border-amber-500/30'
                }`}
              >
        ...
        <div className="relative max-w-2xl mx-auto">
          <div className="relative">
            ...
            <input
              ...
              className="w-full px-5 py-4 pl-12 rounded-2xl bg-white/5 border border-white/10 text-white placeholder-gray-500 focus:outline-none focus:border-amber-500/50 focus:ring-2 focus:ring-amber-500/20 transition-all duration-300"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-4 top-1/2 -translate-y-1/2 p-1 rounded-lg text-gray-500 hover:text-white hover:bg-white/10 transition-all duration-300"
              >
                <X className="w-4 h-4" />
              </button>
            )}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div>
          ...
          <select
            ...
            className="w-full rounded-xl bg-gray-900/80 border border-white/10 px-4 py-3 text-white focus:outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/20 transition-all duration-300 cursor-pointer"
          >
        ...
        <div>
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">Min</span>
              <input
                type="number"
                value={minPrice}
                min={0}
                onChange={(e) => onPriceRangeChange([Number(e.target.value), maxPrice])}
                className="w-full rounded-xl bg-gray-900/80 border border-white/10 px-4 py-3 pl-12 text-white focus:outline-none focus:border-amber-500/50 transition-all duration-300"
              />
            </div>
            <div className="relative flex-1">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">Max</span>
              <input
                type="number"
                value={maxPrice}
                min={0}
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-gray-950/98 backdrop-blur-xl border-l border-white/10 z-50 flex flex-col shadow-2xl shadow-black/50"
          >
              ...
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: -50 }}
                    className="flex gap-3 p-3 rounded-xl bg-white/5 border border-white/5 hover:border-amber-500/30 transition-all duration-300"
                  >
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-white text-sm line-clamp-1">{item.nom}</h4>
                      <p className="text-amber-400 text-sm font-bold">{item.prix.toFixed(2)} TND</p>
                      <div className="flex items-center gap-2 mt-2">
                        <button onClick={() => updateQuantity(item.productId, item.quantite - 1)} className="w-7 h-7 rounded-lg bg-white/10 text-white hover:bg-white/20 transition-all duration-300 flex items-center justify-center">
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="text-sm text-white w-8 text-center font-medium">{item.quantite}</span>
                        <button onClick={() => updateQuantity(item.productId, item.quantite + 1)} className="w-7 h-7 rounded-lg bg-white/10 text-white hover:bg-white/20 transition-all duration-300 flex items-center justify-center">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-white">{(item.prix * item.quantite).toFixed(2)} TND</p>
                    </div>
                  </motion.div>
                ))
              )}
            ...
            {items.length > 0 && (
              <div className="p-5 border-t border-white/10 bg-gradient-to-t from-gray-950/50 to-transparent">
                ...
                <button onClick={onClose} className="w-full mt-3 py-2.5 rounded-xl text-gray-400 hover:text-white transition-all duration-300 text-sm">
