...
export default function OrderTrackingPage() {
  ...
  const getStatusColor = () => {
    if (!order) return 'text-muted-foreground dark:text-gray-400';
    const colorMap: Record<string, string> = {
      'En attente': 'text-muted-foreground dark:text-gray-400',
      'Payée': 'text-blue-400',
      'En préparation': 'text-yellow-400',
      'En livraison': 'text-amber-400',
      'Livrée': 'text-emerald-400'
    };
    return colorMap[order.statut] || 'text-muted-foreground dark:text-gray-400';
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground dark:text-gray-400">Chargement de la commande...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-foreground dark:text-white mb-2">Commande non trouvée</h1>
          <p className="text-muted-foreground dark:text-gray-400 mb-4">{error || "La commande que vous cherchez n'existe pas"}</p>
          <Link href="/espace/mes-commandes" className="text-amber-400 hover:text-amber-700 dark:text-amber-300">
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <Link href="/espace/mes-commandes" className="text-muted-foreground dark:text-gray-400 hover:text-amber-400">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <div>
              <h1 className="text-2xl font-bold text-foreground dark:text-white">Suivi de commande</h1>
              <p className="text-sm text-muted-foreground dark:text-gray-400">Commande #{order._id.slice(-8).toUpperCase()}</p>
            </div>
          </div>
          <button
            onClick={refresh}
            disabled={refreshing}
            className="p-2 rounded-xl bg-black/5 dark:bg-white/5 border border-border dark:border-white/10 text-muted-foreground dark:text-gray-400 hover:text-amber-400 transition-colors"
          >
        ...
        <div className="mb-8 flex justify-center">
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${getStatusColor()} bg-black/5 dark:bg-white/5 border border-border dark:border-white/10`}>
                ...
                return (
                  <div key={step.key} className="flex flex-col items-center flex-1">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      isCompleted ? 'bg-gradient-to-r from-amber-500 to-yellow-400' : 'bg-black/5 dark:bg-white/10 border border-border dark:border-white/20'
                    ...
                    <p className="text-[10px] text-foreground dark:text-gray-500 text-center max-w-[100px] hidden md:block">{step.description}</p>
                  </div>
                );
              })}
            </div>
        ...
        {/* Carte */}
        <div className="rounded-3xl border border-border dark:border-white/10 bg-black/5 dark:bg-white/5 p-6 mb-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Livraison */}
          <div className="rounded-3xl border border-border dark:border-white/10 bg-black/5 dark:bg-white/5 p-6">
            <h2 className="text-lg font-semibold text-foreground dark:text-white mb-4 flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-foreground dark:text-white">{delivery?.adresse_livraison?.adresse || 'Non renseignée'}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs text-foreground dark:text-gray-500 mb-1">Distance</p>
                  <p className="text-sm text-foreground dark:text-white">{delivery?.distance_km?.toFixed(1) || '-'} km</p>
                </div>
                <div>
                  <p className="text-xs text-foreground dark:text-gray-500 mb-1">Frais de livraison</p>
              ...
              {delivery?.chauffeur_nom && (
                <div>
                  ...
                  <p className="text-sm text-foreground dark:text-white">{delivery.chauffeur_nom}</p>
            ...
            <div className="space-y-3">
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {order.items && order.items.length > 0 ? (
                  ...
                  <p className="text-sm text-foreground dark:text-gray-500">Chargement des articles...</p>
                )}
              </div>
              <div className="pt-3 border-t border-border dark:border-white/10 space-y-2">
                <div className="flex justify-between">
                  <span className="text-muted-foreground dark:text-gray-400">Sous-total</span>
                  <span className="text-foreground dark:text-white">{order.montant_total.toFixed(2)} TND</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground dark:text-gray-400">Livraison</span>
                  <span className="text-foreground dark:text-white">{delivery?.cout_livraison?.toFixed(2) || '0.00'} TND</span>
                </div>
                <div className="flex justify-between pt-2 border-t border-border dark:border-white/10 font-bold">
              ...
              <div className="pt-3">
                <p className="text-xs text-foreground dark:text-gray-500 mb-1">Date de commande</p>
                <p className="text-sm text-foreground dark:text-white">{new Date(order.date_commande).toLocaleString('fr-FR')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-center">
          ...
          <p className="text-xs text-foreground dark:text-gray-500 mt-2">Disponible du lundi au vendredi de 8h à 17h</p>
        </div>
      </div>
    </div>
  );
