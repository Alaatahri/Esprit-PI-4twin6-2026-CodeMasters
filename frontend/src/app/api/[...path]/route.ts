...
async function proxy(req: NextRequest, context: Ctx): Promise<NextResponse> {
  ...
  req.headers.forEach((value, key) => {
    ...
    // `expect: 100-continue` (souvent ajouté par certains clients) peut casser le proxy
    // côté Node fetch → on le retire pour éviter des "fetch failed" sur les POST/PATCH.
    if (k === "host" || k === "connection" || k === "expect") return;
