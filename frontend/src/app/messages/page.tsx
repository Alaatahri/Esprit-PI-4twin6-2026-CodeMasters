...
function roleLabel(role?: string): string {
  ...
  if (r === "artisan") return "Contractor";
  return role;
}
    ...
    <div className="mx-auto max-w-2xl px-4 py-8 sm:py-10">
      <div className="flex items-center gap-3 mb-8">
        <div className="rounded-2xl bg-amber-500/15 border border-amber-500/30 p-3">
          <MessageCircle className="w-7 h-7 text-amber-700 dark:text-amber-300" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-foreground dark:text-white">Messages</h1>
          <p className="text-sm text-muted-foreground dark:text-gray-500">
            Your conversations with clients, experts and contractors.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-border dark:border-white/10 bg-white/[0.03] px-6 py-12 text-center space-y-3">
          <p className="text-muted-foreground text-sm">
            No conversations yet. Open a project and use “Contact” to message a client or contractor.
          </p>
          <Link
            href="/espace"
            className="inline-flex text-sm text-amber-400 hover:text-amber-700 dark:text-amber-300"
          >
            Back to dashboard
          </Link>
        </div>
          ...
          {rows.map((c) => (
            <li key={c.partnerId}>
              <Link
                ...
                className="flex items-center gap-4 rounded-2xl border border-border dark:border-white/10 bg-white/[0.04] px-4 py-4 transition hover:border-amber-500/30 hover:bg-white/[0.06]"
              >
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-amber-500/30 to-yellow-500/10 border border-amber-500/20">
                  <User className="w-6 h-6 text-amber-800 dark:text-amber-200" />
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-foreground dark:text-gray-500 uppercase tracking-wide">
                    {roleLabel(c.partnerRole)}
                  </p>
                  <p className="text-sm text-muted-foreground dark:text-gray-400 truncate mt-1">
                    {c.lastBody}
                  </p>
                </div>
