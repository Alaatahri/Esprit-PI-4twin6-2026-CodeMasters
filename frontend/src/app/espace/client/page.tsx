...
export default function ClientSpacePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10">
      {/* Bandeau d'accueil */}
      <header className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-amber-500/[0.08] via-white/[0.04] to-transparent p-6 sm:p-8 lg:p-10">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-amber-500/35 bg-amber-500/15">
              <LayoutDashboard className="h-7 w-7 text-brand dark:text-amber-300" />
            </div>
            <div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row lg:flex-col lg:items-end">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-2xl bmp-btn-primary px-6 py-3 text-sm font-semibold text-gray-900 transition"
            >
            ...
            >
              <Camera className="h-4 w-4 text-brand dark:text-amber-300" />
              Suivi &amp; photos
            </Link>
          </div>
        </div>
        ...
        <div className="relative mt-8 grid grid-cols-3 gap-3 sm:gap-4">
          <div className="rounded-2xl border border-border bg-muted dark:bg-black/20 px-3 py-4 text-center sm:px-4">
          ...
          <div className="rounded-2xl border border-border bg-muted dark:bg-black/20 px-3 py-4 text-center sm:px-4">
            ...
            <p className="mt-1 inline-flex items-center justify-center gap-1.5 text-2xl font-bold text-sky-800 tabular-nums dark:text-sky-300">
          ...
          <div className="rounded-2xl border border-border bg-muted dark:bg-black/20 px-3 py-4 text-center sm:px-4">
            ...
            <p className="mt-1 text-2xl font-bold text-emerald-800 tabular-nums dark:text-emerald-300">
      ...
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] xl:gap-10">
        ...
        <section className="space-y-5">
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <span className="rounded-full border border-border bg-muted px-3 py-1 text-[10px] font-medium uppercase tracking-[0.15em] text-muted-foreground">
              BMP.tn
            </span>
          </div>
          ...
          <div className="space-y-4">
            {exampleProjects.map((ex) => (
              <article
                key={ex.id}
                className="group overflow-hidden rounded-2xl border border-border bg-card transition hover:border-amber-500/25"
              >
                ...
                <div className="space-y-2 p-4 text-sm">
                  ...
                  <p className="border-t border-border pt-2 text-xs italic text-body-secondary">
                    &ldquo;{ex.avis}&rdquo;
                  </p>
                </div>
              </article>
        ...
        <div className="space-y-8">
          <section className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Link
                href="/espace/client/suivi"
                className="inline-flex items-center gap-2 self-start rounded-2xl border border-amber-500/35 bg-amber-500/10 px-3 py-2 text-xs font-medium text-amber-950 transition hover:bg-amber-500/20 dark:text-amber-100"
              >
                Vue suivi complète
            ...
            {loadingProjects ? (
              <div className="flex min-h-[12rem] items-center justify-center rounded-2xl border border-border bg-muted/60">
                <div className="h-9 w-9 animate-spin rounded-full border-2 border-amber-500/30 border-t-amber-400" />
              </div>
            ) : projects.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border bg-muted/60 px-6 py-10 text-center">
                <p className="text-sm text-muted-foreground">
                  Vous n&apos;avez pas encore de projet. Cliquez sur{" "}
                  <span className="font-medium text-brand dark:text-amber-200">
                    Nouveau projet
                  </span>{" "}
              ...
              <div className="max-h-[min(28rem,70vh)] space-y-3 overflow-auto pr-1">
                {projects.map((project) => (
                  <div
                    ...
                    className="cursor-pointer rounded-2xl border border-border bg-gradient-to-br from-white/[0.06] to-transparent px-4 py-3 text-sm shadow-sm transition hover:border-amber-500/35 hover:from-white/[0.08]"
                    onClick={() =>
                  ...
                  >
                    ...
                    <p className="pt-1 text-center text-[10px] text-amber-800/90 dark:text-amber-400/70">
          ...
          <section className="rounded-3xl border border-emerald-500/25 bg-gradient-to-br from-emerald-500/[0.08] via-muted/50 to-background p-5 backdrop-blur-sm dark:from-emerald-950/30 dark:via-transparent dark:to-transparent">
            <div className="mb-4 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <ShoppingCart className="h-5 w-5 text-emerald-800 dark:text-emerald-300" />
            ...
            <div className="space-y-3">
              {[
                ...
                <div
                  key={product.id}
                  className="flex items-center gap-3 rounded-2xl border border-emerald-500/20 bg-muted dark:bg-black/25 px-3 py-3 text-xs"
                >
