...
const SmartMap = dynamic(() => import('@/components/SmartMap'), { 
  ssr: false,
  loading: () => <div className="h-[400px] w-full bg-gray-900 animate-pulse rounded-[2.5rem] flex items-center justify-center text-gray-500">Chargement de l'interface cartographique...</div>
});
    ...
    <div className="p-4 md:p-8 space-y-8 pb-32 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        <div className="flex-1 p-10 rounded-[3rem] bg-gradient-to-br from-amber-500 to-yellow-400 text-gray-950 shadow-2xl shadow-amber-500/20 relative overflow-hidden flex flex-col justify-between min-h-[240px] group">
          ...
          <div className="relative z-10 flex flex-wrap gap-3 mt-8">
            ...
            <div className="px-6 py-3 rounded-2xl bg-white/20 backdrop-blur-md border border-white/10 text-gray-900 font-black text-sm">
              Note : 4.9/5
            </div>
          </div>
        ...
        <div className="xl:w-[400px] grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-1 gap-6">
          <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/10 backdrop-blur-xl flex flex-col justify-between group hover:bg-white/10 transition-all">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Météo Tunis</p>
                  <p className="text-xl font-black text-white">{weather.status}</p>
                </div>
              </div>
              <p className="text-4xl font-black text-white tracking-tighter">{weather.temp}°</p>
            </div>
          ...
          <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/10 backdrop-blur-xl group hover:bg-white/10 transition-all">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Gains du jour</p>
                  <p className="text-xl font-black text-white">245.50 TND</p>
                </div>
              </div>
            ...
            <p className="text-[10px] text-gray-500 mt-2 font-bold uppercase tracking-widest text-right">Objectif : 300 TND</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-4 gap-8">
        ...
        <div className="2xl:col-span-3 space-y-8">
          ...
          <div className="flex items-center justify-between bg-white/5 p-2 rounded-3xl border border-white/10 w-fit">
            <button 
              onClick={() => setActiveTab('missions')}
              className={`flex items-center gap-3 px-8 py-4 rounded-2xl transition-all font-black text-sm ${activeTab === 'missions' ? 'bg-amber-500 text-gray-950 shadow-xl shadow-amber-500/20 scale-105' : 'text-gray-400 hover:text-white'}`}
            >
            ...
            <button 
              ...
              className={`flex items-center gap-3 px-8 py-4 rounded-2xl transition-all font-black text-sm ${activeTab === 'my-deliveries' ? 'bg-amber-500 text-gray-950 shadow-xl shadow-amber-500/20 scale-105' : 'text-gray-400 hover:text-white'}`}
            >
              ...
              >
                {availableMissions.length > 0 ? (
                  availableMissions.map((mission) => (
                    <div key={mission._id} className="p-8 rounded-[2.5rem] bg-white/5 border border-white/10 hover:border-amber-500/40 transition-all group relative overflow-hidden flex flex-col justify-between min-h-[300px]">
                      <div className="flex justify-between items-start mb-6">
                        <div className="w-14 h-14 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500">
                          <Truck className="w-8 h-8" />
                        </div>
                        <div className="text-right">
                          <p className="text-3xl font-black text-white">{mission.cout_livraison || 45} TND</p>
                          <p className="text-[10px] text-emerald-400 font-black uppercase tracking-widest">Payé à 50%</p>
                        </div>
                      </div>

                      <div className="space-y-4 mb-8">
                        <h3 className="text-xl font-bold text-white group-hover:text-amber-500 transition-colors">Mission #{mission.commandeId.slice(-6).toUpperCase()}</h3>
                      ...
                      <div className="flex gap-4">
                        <button 
                          ...
                          className="flex-1 py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-black text-sm hover:bg-white/10 transition-all"
                        >
                          Détails
                        </button>
                        <button 
                  ...
                  <div className="lg:col-span-2 p-24 rounded-[3rem] bg-white/5 border border-dashed border-white/10 flex flex-col items-center justify-center text-center">
                    <div className="w-24 h-24 rounded-full bg-white/5 flex items-center justify-center mb-6">
                      <Package className="w-12 h-12 text-gray-700" />
                    </div>
                    <h3 className="text-2xl font-black text-white">Zone calme</h3>
                    <p className="text-gray-500 max-w-sm mt-2">Nous vous notifierons dès qu'une nouvelle commande est passée dans votre secteur.</p>
                  </div>
                )}
              </motion.div>
            ) : (
              ...
              >
                {activeDelivery ? (
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Live Tracker Map */}
                    <div className="lg:col-span-2 rounded-[3rem] border border-white/10 bg-gray-900 overflow-hidden shadow-2xl relative h-[500px]">
                      <SmartMap delivery={activeDelivery} />
                      <div className="absolute top-6 left-6 right-6 flex justify-between items-start pointer-events-none">
                        <div className="p-5 rounded-[1.5rem] bg-gray-950/80 backdrop-blur-xl border border-white/10 shadow-2xl space-y-3 pointer-events-auto">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-sm font-black text-white">Route la plus rapide via l'A1</p>
                              <p className="text-[10px] text-gray-500 font-bold">Trafic fluide, arrivée prévue à 14:42</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="p-8 rounded-[3rem] bg-white/5 border border-amber-500/20 flex flex-col justify-between backdrop-blur-xl shadow-2xl shadow-amber-500/5">
                      <div>
                        <div className="flex items-center justify-between mb-8">
                          ...
                          <button onClick={() => openMissionDetails(activeDelivery)} className="text-gray-500 hover:text-white transition-colors">
                            <Info className="w-5 h-5" />
                          </button>
                        </div>
                        <h3 className="text-3xl font-black text-white mb-2 leading-tight">#{activeDelivery.commandeId.slice(-6).toUpperCase()}</h3>
                        ...
                        <div className="space-y-4">
                          <div className="p-5 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-between">
                            ...
                            <span className="text-sm font-black text-white">~12 min</span>
                          </div>
                          <div className="p-5 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <Navigation className="w-5 h-5 text-blue-400" />
                              <span className="text-xs font-bold text-gray-400">Distance</span>
                            </div>
                            <span className="text-sm font-black text-white">4.2 km</span>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="p-24 rounded-[3rem] bg-white/5 border border-dashed border-white/10 flex flex-col items-center justify-center text-center">
                    <Truck className="w-12 h-12 text-gray-700 mb-6" />
                    <h3 className="text-2xl font-black text-white">Aucune livraison active</h3>
                    <p className="text-gray-500 mt-2">Sélectionnez une mission dans l'onglet disponible pour démarrer.</p>
                  </div>
                )}
              </motion.div>
            )}
        ...
        <div className="space-y-8">
          ...
          <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/10 backdrop-blur-md">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-lg font-black text-white">Statut</h3>
                <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest">En ligne & Prêt</p>
              </div>
            ...
            {/* Vehicle Card */}
            <div className="p-6 rounded-[2rem] bg-gradient-to-br from-white/[0.08] to-transparent border border-white/10 group hover:border-amber-500/30 transition-all mb-8">
              <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-4">Véhicule Actif</p>
              <div className="flex items-center gap-5">
                ...
                <div>
                  <p className="text-lg font-black text-white leading-none mb-2">{vehicle?.nom || 'Certifié BMP'}</p>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-md bg-gray-950 text-[10px] font-black text-amber-500 border border-white/5">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="p-4 rounded-2xl bg-white/5 border border-white/5 text-center">
                <p className="text-[10px] text-gray-500 uppercase font-black mb-1">Score</p>
              ...
              <div className="p-4 rounded-2xl bg-white/5 border border-white/5 text-center">
                <p className="text-[10px] text-gray-500 uppercase font-black mb-1">Missions</p>
          ...
          <div className="p-8 rounded-[2.5rem] bg-blue-500/10 border border-blue-500/20 relative overflow-hidden">
            ...
            <p className="text-xs text-blue-100/70 leading-relaxed font-medium relative z-10">
              Perturbation majeure signalée sur la <span className="text-white font-bold">Route X-20</span>. Nos algorithmes recommandent le passage par l'avenue Bourguiba pour économiser 8 minutes.
            </p>
            <button className="mt-6 flex items-center gap-2 text-[10px] font-black text-blue-400 uppercase hover:text-white transition-colors">
              Voir sur la carte
        ...
        {selectedMission && (
          ...
          >
            <motion.div 
              ...
              className="bg-gray-900 border border-white/10 rounded-[3rem] w-full max-w-3xl max-h-[90vh] overflow-hidden shadow-[0_0_100px_-20px_rgba(245,158,11,0.2)] flex flex-col"
            >
              {/* Modal Header */}
              <div className="p-10 bg-gradient-to-br from-amber-500/20 to-transparent flex justify-between items-center border-b border-white/5">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-xs font-bold text-gray-500">ID: {selectedMission._id.slice(-8)}</span>
                  </div>
                  <h2 className="text-3xl font-black text-white leading-tight">Commande #{selectedMission.commandeId?.slice(-6).toUpperCase()}</h2>
                </div>
                <button 
                  onClick={() => setSelectedMission(null)}
                  className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all border border-white/5"
                >
                  <X className="w-8 h-8" />
                </button>
              </div>
              ...
              <div className="p-10 overflow-y-auto space-y-10 custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="space-y-8">
                    <div className="space-y-4">
                      ...
                      <div className="p-6 rounded-3xl bg-white/5 border border-white/5 space-y-4">
                        <div className="flex gap-4">
                          ...
                          <div>
                            <p className="text-[10px] text-gray-500 font-black uppercase">Ramassage</p>
                            <p className="text-sm font-bold text-white">Entrepôt Central BMP - Tunis</p>
                          </div>
                        </div>
                        <div className="flex gap-4">
                          ...
                          <div>
                            <p className="text-[10px] text-gray-500 font-black uppercase">Arrivée</p>
                            <p className="text-sm font-bold text-white leading-snug">{selectedMission.adresse_livraison}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-5 rounded-2xl bg-white/5 border border-white/5 text-center">
                        <p className="text-[10px] text-gray-500 font-black mb-1">Distance</p>
                        <p className="text-lg font-black text-white">{selectedMission.distance_km || 12.4} km</p>
                      </div>
                      <div className="p-5 rounded-2xl bg-white/5 border border-white/5 text-center">
                        <p className="text-[10px] text-gray-500 font-black mb-1">Durée Est.</p>
                  ...
                  <div className="space-y-8">
                    <div className="space-y-4">
                      ...
                      <div className="p-6 rounded-3xl bg-emerald-500/5 border border-emerald-500/10 space-y-6">
                        <div className="flex justify-between items-center">
                          <p className="text-xs font-bold text-gray-400">Mode :</p>
                        ...
                        <div className="flex justify-between items-end">
                          <div>
                            <p className="text-[10px] text-gray-500 font-black uppercase">À encaisser</p>
                          ...
                          <div className="text-right">
                            <p className="text-[10px] text-gray-500 font-black uppercase">Votre gain</p>
                    ...
                    <div className="space-y-4">
                      <h4 className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Articles Transportés</h4>
                      <div className="space-y-2 max-h-32 overflow-y-auto pr-2 custom-scrollbar">
                        {orderDetails?.items?.map((item: any, i: number) => (
                          <div key={i} className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5">
                            <span className="text-xs font-bold text-gray-300">{item.produitNom || 'Produit'}</span>
                            <span className="text-xs font-black text-white">x{item.quantite}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  <h4 className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Aperçu du Trajet</h4>
              ...
              {!activeTab.includes('my-deliveries') && (
                <div className="p-10 bg-white/5 border-t border-white/5 flex gap-6">
                  ...
                  <button 
                    onClick={() => setSelectedMission(null)}
                    className="flex-1 py-6 rounded-2xl bg-white/5 border border-white/10 text-white font-black text-lg hover:bg-white/10 transition-all"
                  >
                    Fermer
                  </button>
                </div>
