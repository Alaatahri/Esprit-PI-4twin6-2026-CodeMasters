  ...
  return (
    <div className="p-6 md:p-10 space-y-10 pb-32 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        <div>
          <h1 className="text-3xl font-black text-foreground dark:text-white flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-amber-500 flex items-center justify-center">
              <Package className="w-7 h-7 text-gray-950" />
            </div>
            Flux de Missions
          </h1>
          <p className="text-foreground dark:text-gray-500 mt-2 font-medium">Découvrez les opportunités de livraison en temps réel dans votre zone.</p>
        </div>

        <div className="flex flex-col sm:flex-row items-center gap-4">
          <div className="relative group w-full sm:w-80">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-foreground dark:text-gray-500 group-focus-within:text-amber-500 transition-colors" />
            <input 
              type="text" 
              placeholder="Filtrer par zone ou adresse..." 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-12 pr-4 py-4 rounded-2xl bg-black/5 dark:bg-white/5 border border-border dark:border-white/10 text-sm text-foreground dark:text-white focus:outline-none focus:border-amber-500/50 focus:ring-4 focus:ring-amber-500/5 transition-all"
            />
          </div>
          <button className="w-full sm:w-auto px-6 py-4 rounded-2xl bg-black/5 dark:bg-white/5 border border-border dark:border-white/10 text-muted-foreground dark:text-gray-400 hover:text-foreground dark:text-white hover:bg-black/5 dark:bg-white/10 transition-all flex items-center justify-center gap-2">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-80 rounded-[2.5rem] bg-black/5 dark:bg-white/5 border border-border dark:border-white/10 animate-pulse" />
          ))}
        </div>
      ) : filteredMissions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 text-center">
          <div className="w-24 h-24 bg-black/5 dark:bg-white/5 rounded-full flex items-center justify-center mb-8 border border-dashed border-border dark:border-white/10">
            <Package className="w-12 h-12 text-gray-700" />
          </div>
          <h3 className="text-2xl font-black text-foreground dark:text-white">Aucune mission trouvée</h3>
          <p className="text-foreground dark:text-gray-500 max-w-sm mt-2 font-medium">Revenez un peu plus tard ou élargissez votre périmètre de recherche.</p>
        </div>
      ) : (
          ...
          <AnimatePresence mode="popLayout">
            {filteredMissions.map((mission) => (
              <motion.div
                ...
                className="bg-black/5 dark:bg-white/5 border border-border dark:border-white/10 rounded-[2.5rem] overflow-hidden group hover:border-amber-500/40 transition-all shadow-2xl hover:shadow-amber-500/5 flex flex-col"
              >
                <div className="p-8 space-y-8 flex-1">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] text-foreground dark:text-gray-500 uppercase font-black tracking-widest">Type de mission</p>
                        <p className="text-lg font-bold text-foreground dark:text-white">Transport Standard</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-3xl font-black text-emerald-400 tracking-tighter">{mission.cout_livraison || 45}.00 TND</p>
                      <p className="text-[10px] text-foreground dark:text-gray-500 uppercase font-bold tracking-widest">Paiement Garanti</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground dark:text-gray-300 font-bold leading-snug line-clamp-2">
                  ...
                  <div className="grid grid-cols-3 gap-6 p-6 rounded-3xl bg-black/5 dark:bg-white/5 border border-border dark:border-white/5">
                    <div className="text-center">
                      <p className="text-[10px] text-foreground dark:text-gray-500 uppercase font-black mb-2 tracking-widest">Distance</p>
                      <div className="flex items-center justify-center gap-1.5">
                        <TrendingUp className="w-3.5 h-3.5 text-amber-500" />
                        <p className="text-sm font-black text-foreground dark:text-white">{mission.distance_km || 12.4} km</p>
                      </div>
                    </div>
                    <div className="text-center border-x border-border dark:border-white/5">
                      <p className="text-[10px] text-foreground dark:text-gray-500 uppercase font-black mb-2 tracking-widest">Volume</p>
                      <p className="text-sm font-black text-foreground dark:text-white">Standard</p>
                    </div>
                    <div className="text-center">
                      <p className="text-[10px] text-foreground dark:text-gray-500 uppercase font-black mb-2 tracking-widest">Temps</p>
                      <div className="flex items-center justify-center gap-1.5">
                        <Clock className="w-3.5 h-3.5 text-blue-400" />
                        <p className="text-sm font-black text-foreground dark:text-white">25 min</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <button 
                      ...
                      className="flex-1 py-5 rounded-2xl bg-black/5 dark:bg-white/5 border border-border dark:border-white/10 text-foreground dark:text-white font-bold hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/30 transition-all flex items-center justify-center"
                    >
                      <X className="w-6 h-6" />
                    </button>
                  </div>
