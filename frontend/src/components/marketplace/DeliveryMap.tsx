...
export function DeliveryMap({ 
  ...
  if (!isClient) {
    return (
      <div style={{ height, minHeight: '400px' }} className="w-full rounded-xl overflow-hidden border border-white/10 bg-gray-800/50 flex items-center justify-center">
  ...
  return (
    <div className="relative" style={{ height }}>
      <div 
        ...
        className="relative w-full h-full cursor-crosshair rounded-xl overflow-hidden border border-white/10"
      ...
      >
        ...
        {/* Instructions */}
        <div className="absolute top-3 left-3 right-3 bg-black/70 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm text-center">
          📍 Cliquez n'importe où sur la carte pour sélectionner votre adresse
        </div>
      </div>
      
      {(selectedLat && selectedLng) && (
        <div className="absolute bottom-3 left-3 right-3 bg-emerald-500/90 backdrop-blur-sm rounded-lg px-4 py-2 text-white text-sm text-center">
