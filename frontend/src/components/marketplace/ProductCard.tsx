...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -8 }}
      className="group relative rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl overflow-hidden hover:border-amber-500/30 transition-all duration-300"
    >
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <span className="text-2xl font-bold text-amber-400">
              {product.prix.toFixed(2)} <span className="text-sm font-normal text-gray-500">TND</span>
            </span>
            {product.stock > 0 && (
      ...
      {showNotification && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: 20 }}
          className="absolute bottom-20 left-4 right-4 bg-emerald-500 text-white text-sm font-medium py-2 px-3 rounded-lg text-center z-20"
        >
          Ajouté au panier !
        </motion.div>
      )}
