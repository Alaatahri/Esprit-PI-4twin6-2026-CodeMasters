    ...
    <div className="space-y-6">
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.key} className="flex flex-col items-center">
                ...
                <p className={`text-xs font-medium mt-2 ${isCompleted ? 'text-amber-400' : 'text-gray-500'}`}>
                  {step.label}
                </p>
      ...
      <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-white/5 border border-white/10">
        <div>
          ...
          <p className="text-sm text-white">{delivery.adresse_livraison}</p>
        </div>
        ...
        {delivery.livreur_nom && (
          <>
            <div>
              ...
              <p className="text-sm text-white">{delivery.livreur_nom}</p>
            </div>
        ...
        {delivery.tracking_number && (
          <div className="col-span-2">
            ...
            <code className="text-sm text-white bg-black/30 px-2 py-1 rounded">{delivery.tracking_number}</code>
          </div>
        )}
      </div>
    </div>
