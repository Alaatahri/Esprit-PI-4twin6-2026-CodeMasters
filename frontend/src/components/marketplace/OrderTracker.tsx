    ...
    <div className="space-y-6">
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.key} className="flex flex-col items-center flex-1">
                ...
                <p className="text-[10px] text-foreground dark:text-gray-500 text-center max-w-[80px] hidden sm:block">{step.description}</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-black/5 dark:bg-white/5 border border-border dark:border-white/10">
        <div>
          ...
          <p className="text-sm text-foreground dark:text-white">{new Date(createdAt).toLocaleString('fr-FR')}</p>
        </div>
        {estimatedDelivery && (
          <div>
            <p className="text-xs text-foreground dark:text-gray-500 mb-1">Livraison estimée</p>
