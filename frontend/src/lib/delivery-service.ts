...
class DeliveryService {
  ...
  async getNearbyDrivers(
    ...
    try {
      ...
      const realVehicles = await marketplaceAPI.getAvailableVehicles();
      ...
      const drivers: Driver[] = realVehicles.map((v: any) => ({
        _id: v._id,
        nom: v.chauffeur_nom || v.nom || "Chauffeur",
        telephone: v.chauffeur_telephone || "+216 00 000 000",
        note: typeof v.note_moyenne === "number" ? v.note_moyenne : 4.5,
        vehicule: {
          type: v.type,
          capacite_tonnes: typeof v.capacite_tonnes === "number" ? v.capacite_tonnes : 1.5,
          immatriculation: v.immatriculation
        },
        position_actuelle: v.position_actuelle || { lat: pickupLat, lng: pickupLng },
        distance_km: 0,
        prix_livraison: 0,
        temps_arrivee_min: 0,
        disponible: Boolean(v.disponible) && Boolean(v.statut_disponible ?? true),
        nombre_courses: v.nombre_courses || 10,
        prix_par_km: typeof v.prix_km === "number" ? v.prix_km : (this.PRIX_PAR_KM[v.type] || 2.0)
      }));
