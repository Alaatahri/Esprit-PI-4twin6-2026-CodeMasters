...
async function run() {
  ...
  for (const p of projects) {
    ...
    await db.connection.collection('projects').updateOne(
      { _id: p._id },
      { $set: { photosApres: [photoApres], photosAvant: [photoAvant] } }
    );
  }
  ...
  for (const s of suivis) {
    if (s.photo_url || s.photoUrl) {
      ...
      await db.connection.collection('suiviprojects').updateOne(
        { _id: s._id },
        { $set: { photo_url: img, photoUrl: img } }
      );
    }
  }
