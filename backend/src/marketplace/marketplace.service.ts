...
async createAvis(createAvisDto: any): Promise<Avis> {
  ...
  if (createAvisDto.workerId) {
    try {
      avisData.workerId = new Types.ObjectId(createAvisDto.workerId);
      avisData.type = 'worker';
    } catch (e) {}
  }
...
async getAvisService(): Promise<Avis[]> {
  /** Liste complète (service, produit, livreur, worker) pour le tableau « Avis » du marketplace */
  return this.avisModel.find().sort({ createdAt: -1 }).exec();
}

/** Avis marketplace affichés sur le profil public d’un artisan / expert */
async getAvisForWorkerPublicProfile(workerUserId: string): Promise<Avis[]> {
  if (!Types.ObjectId.isValid(workerUserId)) return [];
  const oid = new Types.ObjectId(workerUserId);
  const vehicles = await this.vehicleModel
    .find({ userId: workerUserId })
    .select('_id')
    .lean()
    .exec();
  const vehicleIds = vehicles.map((v) => v._id);

  const orConds: Record<string, unknown>[] = [{ workerId: oid }];
