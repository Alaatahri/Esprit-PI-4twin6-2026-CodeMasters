...
async getTopProduits() {
  ...
  estimatePrice(@Body() data: { type: string; quantite: number; qualite?: string }) {
  ...
  calculateDeliveryPrice(
    @Body() data: { produitId: string; destinationLat: number; destinationLng: number; vehicleId?: string }
  ) {
...
async analyserBesoin(@Body() data: { 
  description: string; 
  surface?: number; 
  type_projet?: string 
}) {
...
async analyserImage(@Body() data: { image: string }) {
